...
export default function ResultsDisplay() {
  ...
  */
  ...
  const [answerFormat, setAnswerFormat] = useState<resultsUtils.answerData>({
    'questionId' : 0,
    'questionText' : '',
    'questionBody' : '',
    'questionCategory' : '',
  ...
  const [resultsFormat, setResultsFormat] = useState<resultsUtils.resultData>({
    'resultId' : resultNum,
    'attemptId' : attemptNum,
    'totalScore' : 0,
    'entranceLevel' : '',
    ...
    if (questions !== undefined) {
      // Note, this can also be written as async function fetchTestFormat() {...}. It's called an arrow function here.
        const fetchData = async () => {
          ...
          const fetchedAnswerFormat = await resultsUtils.answersData('retrieveAnswers', answerFormat);
          const fetchedResultsFormat = await resultsUtils.resultsData('retrieveResults', resultsFormat);
